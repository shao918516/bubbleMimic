...
class RewardsCfg:
    ...
    # 若脚踝终止占比仍长期 90%+ 可再上调。
    # 【第二步实验:基准(零课程+entropy_coef=0.01)之上只加这一个变量】
    # 上次此项失败时配的是 entropy_coef=0.005,两变量同时改无法归因;这次配 0.01
    # 是干净的单变量对照。权重先保持 0.10 不上调——0.10+0.01 本身就没测过,直接
    # 加到 0.2 会让它超过 anchor_pos(0.15) 成为最大正向项,干扰根部的风险明显变大。
    # 决策线:iter100 根部误差≈0.24、iter200≈0.12 → 健康,继续跑看脚踝终止占比;
    # 根部停在 0.5+ → 立刻停,锅明确是此项,回退基准 checkpoint。
    # 若根部健康但脚踝改善不明显,下一轮再把 weight 升到 0.15~0.2。
...
class CurriculumCfg:
    ...
    【当前方案：零课程学习。两个终止阈值全程固定 0.4，不做任何放宽。】

    历史结论（多轮实验后确定，不要轻易再加课程回来）：
    - 零课程 + 阈值 0.4 是唯一一次让根部误差学到 0.12 的配置（iter400 验证）。
    - ee_body_pos 阈值课程先后试过 4 个版本（快/慢、配不配 anchor_pos 课程），
      每一次 error_anchor_pos 都停滞在 0.5~0.7、anchor_pos 终止占比 98%+，
      比零课程差 5 倍。脚部宽松期对根部学习的干扰是物理层面的（脚踩得歪根部就
      稳不住），不通过阈值判定传导，所以调 anchor_pos 那边的课程参数救不了。
    - 脚踝（ee_body_pos 98% 终止、episode 短）这个真正的瓶颈，这一版改为从
      reward 侧解决：见 RewardsCfg 里新增的 motion_ankle_pos 专项追踪，
      不再走终止阈值课程这条已经证伪的路。

    本版配套设定：entropy_coef=0.005（rsl_rl_ppo_cfg.py），零课程 + 0.005 +
    脚踝专项 reward，是本轮要验证的组合。
    """
